...
@section('content')
    ...
    <div class="py-5">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('teacher.student.edit', $student->id) }}">
                @csrf
                ...
                <div>
                    ...
                    <x-text-input id="email" class="block mt-1 w-full" type="email" name="email" :value="$student->email"
                                  required autofocus autocomplete="email"/>

                    <x-input-label for="id_no" class="mt-2" :value="__('ID Number')"/>
                    <x-text-input id="id_no" class="block mt-1 w-full" type="text" name="id_no" :value="$student->id_no"
                    ...
                    <x-text-input id="school_no" class="block mt-1 w-full" type="text" name="school_no"
                                  :value="$student->school_no"
                                  required autofocus autocomplete="school_no"/>

                    <x-input-label for="grade_id" class="mt-2" :value="__('Grade')"/>
                    <select id="grade_id" name="grade_id"
                        ...
                        @foreach($grades as $grade)
                            <option value="{{ $grade->id }}" {{ $student->grade_id == $grade->id ? 'selected' : null }}>{{ $grade->name }}</option>
                        @endforeach
                    </select>
